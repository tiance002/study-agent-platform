...
以下三条来自一次**自查**（不是外部审查），都已修或已明确标注：

- **`supported` 的含义比它的名字弱**（未修，已标注）：规格说「核心结论有充分且一致的证据」，
  但首版只在**没有任何 issue** 时返回它 —— 检查的是"检索过程没发现异常"
  （有候选、相关度达标、必需步骤完成、无抓取失败），**不是**"核心结论已被验证"。
  后者同样需要那套冻结的核心结论标注集。这与 `partially_supported` 是同一类问题的两个方向：
  那里是把不确定性讲小（因此收紧为默认不产生），这里是把能力边界讲大（只能诚实标注）。
  没有一并收紧是因为：要求正向结论覆盖才返回 `supported`，首版会让每次检索都判成
  `insufficient`，状态退化成常量 —— 那既没用，也会掩盖真正的不足。
  标注集就绪后要给 `supported` 加正向校验，测试里留了一条**绊线**
  （`test_supported_means_no_detected_gap_not_verified_conclusions`）防止这条被忘掉。
