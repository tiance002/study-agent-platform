...
- [ ] 先执行第四轮审查 R4-01..07 修复，独立通过回归；修复提交与第五轮功能提交分开。
- [ ] 测试迁移和应用 DSN 均指向独立随机测试库；错误指向 study_platform 时拒绝 setup 写入。
- [ ] 不因全量绿色自动判定退出；退出依据下表的具体性质及证据。
...
文件：审查文档涉及的七组文件；新增前向迁移 `0008_ingestion_integrity.py`，同步 `main.py` schema 版本和生成契约。

- [ ] 先建立测试库隔离，避免修复过程破坏用户资料。
- [ ] 分别复现 worker 角色绕过、旧租约回写、多版本错引、同长伪片段、纯文本丢失、围栏长度错误。
- [ ] 按审查要求修复；不修改已部署 0007 代替前向迁移。
- [ ] 运行两个 worker、同 worker_id 重启、连接复用、资料多版本组合测试；修复后旧引用继续可读。
- [ ] 完成 `0007→0008→0007→0008` 临时库演练，并定义历史不可验证片段的标记/重建策略。
- [ ] 提交 `fix: close round 4 review findings`，独立审查后才进入任务 1。
...
- [ ] 阅读现有 00/02/03/04/06 规格；核验选中 provider 官方文档，将模型标识、SDK 版本、输入/输出上限、超时、幂等支持、usage 字段与查询/对账能力记入决策记录。
- [ ] 确认用户资料外发授权范围，提供明确功能开关；凭据仅服务端读取，日志/异常/数据库不保存秘密。
- [ ] 定义以下领域端口，供应商 SDK 只出现在适配器：
...
- [ ] 编写 ScriptedProvider 模拟响应、明确拒绝、派发前失败、派发后超时、畸形 JSON、截断、缺 usage、错误引用；记录调用次数而非只返回固定成功文本。
- [ ] 禁止 SDK 隐藏自动重试；若供应商幂等有明确定义，重试只能复用同一个 durable attempt 标识并验证其期限。
- [ ] 对缺配置、非法模型、越界输入先拒绝，证明调用次数为 0；超时与明确失败返回不同领域结果。
- [ ] 完成定向测试与提交 `feat: define teaching provider boundary`。
...
- [ ] 先冻结表关系：teaching_runs 保存问题引用、状态、fencing token、模型/prompt/检索版本、最终消息引用；provider_attempts 保存派发状态与外部请求 id；teaching_events 保存稳定事件序号；预算账户/预留/结算保存额度事实。
- [ ] 原子命令 `start_run(actor, project_id, conversation_id, question)` 同事务写 user message、run、预算预留及对应意图；其中任何写入失败都回滚。HTTP 缓存写回未知仍遵循现有 reconciliation 语义。
- [ ] `claim_run(worker_context)` 复用任务 0 建立的真正 worker 角色边界；普通应用 SQL 角色不能自行设置变量读取全局 runs。
- [ ] `mark_dispatched(claim, attempt_id)` 在调用 provider 前持久化，原子把预算 held→in_flight；网络调用不持有数据库事务或行锁。
- [ ] 预算预留同时限制 tenant 总额度、项目额度、输入与最大输出；价格快照有版本，整数微单位记账，禁止浮点累计。估计上界必须采用供应商可证明上界或经过验证的 tokenizer；超上界作为明确异常保守记账，不静默截断消费值。
- [ ] 缺 usage 的成功结果不得按零费用释放；保留待对账状态与费用敞口。
- [ ] 用双连接屏障测试“最后一份额度同时争抢”，只有一个命令能成功；重启后额度、预留、运行状态不丢。
- [ ] 在 user message 后、run 后、预算后分别故障注入，验证无部分写入；测试 RLS、组合外键和列级权限。
- [ ] 提交 `feat: persist teaching intents and budget reservations`。
...
- [ ] 输入上下文只含本用户获授权项目与会话历史；分别限制原问题、历史、资料数量和总 token；超限显式裁剪策略或拒绝，不能截掉租户/权限约束。
- [ ] 外部资料作为带 taint 的数据片段，固定 system 指令由服务端产生；资料中的“忽略规则”“调用工具”不改变权限或 provider 请求工具集。
- [ ] retrieval snapshot 冻结 document_id、chunk_id、span、hash、parser_version、ranking_version 和 prompt_version；重启继续使用同一快照，不悄悄换成新版来源。
- [ ] 分开内部是否可外发与用户 display_policy；citation_only 不向用户输出正文，未批准外发的正文不进 provider；summary 不能简单返回原文全文。
- [ ] 校验返回引用是本次已授权 snapshot 的子集，重新确认原文跨度与 hash；伪造/跨项目/未知版本引用拒绝 final commit。
- [ ] 机械引用匹配只证明出处存在，不能证明结论得到支持。没有独立语义评估时维持保守 evidence 状态；明确区分 sourced 与 inference，禁止模型自行宣称 mastered/supported。
- [ ] 无资料时可以给标注为一般性说明的回答，或明确资料不足；不得编造引用、把 provider 升级当成证据充分。
- [ ] 校验失败不自动无限重试；首版一次生成，记录校验失败，费用按已发生调用结算。
- [ ] 提交 `feat: validate teaching context and citations`。

## 任务 4：worker 执行与故障恢复

新增：`backend/app/teaching/service.py`、`backend/app/workers/teaching.py`、`backend/tests/test_teaching_recovery.py`。

- [ ] 实现 load snapshot→校验授权→预算持久化派发→provider→输出校验→原子 final commit。
- [ ] `finish_run(claim, attempt_id, result)` 同事务写唯一 assistant message、费用结算、终态事件和 run succeeded；失败不得出现“已成功但无消息”。
- [ ] 对派发前确定失败释放 held；派发后结果未知保留 in_flight，转 reconciliation_required，禁止自动再次 generate。
- [ ] worker 死在“写 dispatched 后、真正发出前”也按未知保守处理；除非供应商可证明未发生，不能从缺响应推断未调用。
- [ ] final commit 失败后允许重放已持久化结果落库，不能通过重新调用模型修复；结果尚未持久化则走 provider 可验证恢复或对账。
- [ ] 用户可查询安全错误和下一动作，原始 SDK 异常/密钥/全文不进入公共错误或普通日志。
- [ ] 逐项执行下方故障矩阵，检查供应商调用计数、消息数量和预算余额三者，不只检查 HTTP 状态。
- [ ] 提交 `feat: execute teaching runs with recoverable outcomes`。
...
- [ ] 新建 run 复用 cookie/CSRF/idempotent_write；同键同体返回同一 run，冲突体拒绝，跨 conversation 不可串用命令作用域。
- [ ] 状态查询和 SSE 都验证成员关系；缺失、跨用户项目、跨租户统一 404。授权应在每次连接及后续读取批次检查，撤销后停止继续发内容。
- [ ] 事件序号由库分配，SSE event id 来源于持久化序号；Last-Event-ID 非法/超界明确处理，心跳不产生业务消息。
- [ ] SSE 断开只结束订阅；worker 继续处理，重连不触发 generate、不再创建 run。事件投递允许至少一次，客户端按 event id 去重。
- [ ] 只发状态和校验完成的 final 事件；最终内容通过已存储 message 读取。慢客户端有明确读批次与连接资源上限。
- [ ] 完成端到端测试：POST 202→订阅→断线→重新装配平台→重连→同一最终消息与费用。
- [ ] 提交 `feat: expose teaching runs and durable event replay`。
...
- [ ] 先冻结至少 30 个验收样例：有资料、无资料、冲突版本、错误引用、提示注入、中文技术解释；期望由人工按原文编写，不能由被测模型自判生成。
- [ ] 结构与隔离硬门要求全部通过；语义引用支持率和教学质量单独评估，阈值在第一次正式评测前冻结，不把关键词命中作为语义正确率。
- [ ] 对租约 token、引用版本、原文比较、预算原子性做定向变异，证明每个移除保护的版本都被对应反例击败，再恢复并确认工作区无变异残留。
- [ ] 实际 provider 小规模验收前核对配置与预算上限；未提供可用凭据时记录外部阻塞，只能标为“模拟器/持久化已验收”，不能标整轮真实模型完成。
- [ ] 在独立 PG 测试库运行第五轮端到端与全量回归，拒绝 PG skip；完成新迁移往返和故障恢复演练。
- [ ] 运行以下仓库门禁并检查退出码，不因后一个命令成功忽略前一个失败：
...
- [ ] 记录审查基线 SHA、各反例的实际结果、测试数量/跳过数、真实 provider 调用数、预算消耗、明确局限。
- [ ] 独立审查在阅读实现之前按矩阵设计反例；未关闭 P1 或与退出门相关 P2 不宣告完成。
- [ ] 按既有授权每轮提交并推送 GitHub，先核对远端新增修改；API 快照工具需要 `--repo tiance002/study-agent-platform --branch main`，推送后核对 SHA/tree。
