...
> 状态（2026-09-24）：第八轮 C1/C2/C3、真实 Web/worker 重启恢复、未知结果与 R8-G 多视口/键盘闭环均已取证。第九轮 acquisition、HTML provenance、可选本地查询改写、混合检索增量（版本化键 + 确定性 RRF + 降级闭集）与检索模式指标（0019）已有实现并过全量门禁；P9 审查修复已落（指标行过滤、CHECK 类型约束、索引故障降级、scope 键隔离、快照版本冻结、前端文案区分）。仍未完成：目标 ECS 扩展安装（生产变更，须用户确认）、前端浏览器矩阵回归、0019 指标快照性能复测、生产发布门。
...
- [x] 教学回答展示检索模式（关键词/混合/降级）、资料版本和引用原文；不把“搜索到”写成“事实已验证”。（2026-09-24：run 响应含 `retrieval_decision`，证据栏显示检索模式与降级原因（P9 后已区分“查询改写不可用”与“向量检索回退”两种降级文案，未知原因固定中文兜底）；资料版本（document_id）在资料列表展示、证据栏显示引用跨度与哈希，二者合计覆盖“版本 + 原文”验收语义；grounding 文案区分“已核验/未核验”。**验收缺口：旧 run、三种降级原因与移动端证据栏的浏览器矩阵回归未跑。**）
...
- [x] 新增 fetch latency、bytes、attempt、provider usage、route decision、fallback 和 reconciliation 指标；正文、API key、完整 prompt 默认不进日志。（0017/0018 覆盖前六类；2026-09-24 补 0019 检索模式/降级原因聚合与 `render_metrics` 白名单渲染 —— P9 修复后只统计合法整行标签组合，闭集标签，无动态标签透传。**口径与验收缺口：聚合来源是已持久化 `retrieval_decision` 的已派发教学 run（HELP 已注明），不代表全部检索尝试；0018 的百万级快照性能数据不覆盖 0019 新增的 JSONB 分组，需复测。**）
