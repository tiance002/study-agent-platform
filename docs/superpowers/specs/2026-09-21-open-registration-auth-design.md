...
> **历史文档（归档）**：本文是 `0012` 开工时的实现前规格，其中"保留邀请码兑换"
> "密码 12–128""`user_sessions.auth_method`"等约定**已被 2026-09-25 新版改造取代**
> （见 `docs/superpowers/plans/2026-09-25-v2-clean-baseline-and-auth-rebuild.md`）：
> 邀请码全链路已删除、密码统一为 6–12 码点、会话不再有 `auth_method`。
> 下文仅作历史决策证据保留，不再描述当前行为。
