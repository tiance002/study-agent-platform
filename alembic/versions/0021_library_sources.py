...
⚠️ **`down_revision` 目前临时指向 `0019`**：并行的另一条轨道会新增
`0020`（learning_tasks 加列）。集成时把本迁移改成 `down_revision = "0020"`
即可 —— 这条是刻意标注的临时状态，不是遗漏。
...
revision = "0021"
#: ⚠️ 临时值 —— 并行轨道的 `0020`（learning_tasks 加列）集成时，这里改成 "0020"。
down_revision = "0019"
branch_labels = None
depends_on = None
