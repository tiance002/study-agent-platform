...
`claim_next` 是刻意的例外：**worker 必须先发现"哪个租户有活干"，才能建立
任何租户上下文**。要求它带身份，等于要求它在知道自己要处理谁之前就声称
自己是谁 —— 那只能靠伪造一个身份来满足签名。
...
class IngestionRepository(Protocol):
    ...
    def enqueue_with_source(
        ...
        *,
        ...
        """在同一事务内"登记或复用项目资料 + 入队摄取"，返回 `(资料, 任务, 是否新建资料)`。

        这是知识库关联路径的原子命令。为什么不能拆成两次调用（先
        `products.register_source` 再 `enqueue`）：第二步失败会留下
        "有资料、没有摄取任务"的半成品 —— 而且它看起来完全合法，
        用户看到的是"关联成功但永远搜不到"。

        幂等口径与 `enqueue` 一致，另加两点：

        - 资料按 `(project_id, identity_hash)` 复用：命中既有资料时返回它，
          `created=False`，不会插入第二行；
        - 若该项目里这条资料**已有未失败的摄取任务**，直接复用该任务、
          不重复入队（重复关联不产生第二个摄取）。失败的任务不算：
          重关联正是用户表达"再试一次"的方式。

        `media_type` 是**资料登记**的元数据（自由文本），`document_media_type`
        是**摄取**接受的闭集之一。两者不是同一个问题，故意分开。
        """
        ...
